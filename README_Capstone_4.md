...
LLM query decomposition
        │
        ├──► sub-query 1 ──► active retriever ──┐
        ├──► sub-query 2 ──► active retriever ──┼──► score accumulation
        └──► sub-query n ──► active retriever ──┘
                                                │
                                                ▼
                                      multi-step evidence
                                                │
                         ...
                         │                                             │
                         ▼                                             ▼
                  baseline seeds                              Wikipedia graph
...
the script.
  ...
  ----------------------------- -------------------------------- -----------------------
  `WIKIPEDIA_TEXT_DIR`          `Wikipedia_10_text`              Wikipedia text corpus

  `WIKIPEDIA_CHROMA_DIR`        `Wikipedia_chroma_topics_full`   Chroma vector database

  `WIKIPEDIA_TOPICS_METADATA`   `wikipedia_topics_10.json`       Category/topic metadata
  ...
  `WEIGHT_BM25`                 `0.5`                            BM25 contribution to
                                                                 hybrid retrieval

  `WEIGHT_VECTOR`               `0.5`                            Vector contribution to
                                                                 hybrid retrieval

  `RETRIEVER_MODE`              `hybrid_tagged`                  Active baseline
                                                                 retriever

  `DECOMPOSITION_MODEL`         `deepseek-coder-6.7b-instruct`   Query decomposition LLM
...
### Graph schema
  ...
  ----------------------- ----------------------------------- -----------------------
  `article`               `doc:Hallstatt.txt`                 Represents a Wikipedia
                                                              article

  `category`              `category:place_geography`          Represents a broad
                                                              retrieval category

  `topic`                 `topic:academy awards ceremonies`   Represents a Wikipedia
